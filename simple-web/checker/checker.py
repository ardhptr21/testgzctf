...
def check_service(target):
    ...
    response = target.get("/")
    if response.status_code != 200 or "Greeting Preview" not in response.text:
        raise Mumble("index page failed")
    response = target.post("/preview", data={"name": "checker", "template": "Hello {{ name }}!"})
    if response.status_code != 200 or "Hello checker!" not in response.text:
        raise Mumble("preview flow failed")
